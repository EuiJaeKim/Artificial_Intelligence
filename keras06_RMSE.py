...
model.add(Dense(23,input_dim = 1, activation = 'linear'))
model.add(Dense(19))
model.add(Dense(35))
model.add(Dense(75))
model.add(Dense(1))
...
#3. 훈련
model.compile(loss='mse',optimizer='rmsprop',metrics=['accuracy']) #기계어로 번역
...
# R2 (알스퀘어) 결정계수
# 통계학에서, 결정계수는 추정한 선형 모형이 주어진 자료에 적합한 정도를 재는 척도이다. 반응 변수의 변동량 중에서 적용한 모형으로 설명가능한 부분의 비율을 가리킨다. 결정계수의 통상적인 기호는 이다.
